...
def get_config():
    ...
    parser.add_argument('--eval_batch_size', type=int, default=100, help='number of dialogue turns in a batch for evaluation')
    ...
    # output file path
    parser.add_argument('--corpus_dst_result', type=str, default='', help='DST output of corpus interaction')
    parser.add_argument('--corpus_act_result', type=str, default='', help='Policy output of corpus interaction')
    parser.add_argument('--corpus_word_result', type=str, default='', help='NLG output of corpus interaction')
    parser.add_argument('--usr_dst_result', type=str, default='', help='DST output of agent-agent interaction')
    parser.add_argument('--usr_act_result', type=str, default='', help='Policy output of agent-agent interaction')
    parser.add_argument('--usr_word_result', type=str, default='', help='NLG output of agent-agent interaction')
    parser.add_argument('--print_sample', type=int, default=1000, help='the number of printed dialogues')

    # network hyper-parameter
    parser.add_argument('--train_size', type=int, default=10000, help='max number of training dialogues used')
    ...
    parser.add_argument('--oracle_dst', type=str2bool, default=False, help='true for oracle belief state, otherwise the predicted belief state')
    parser.add_argument('--usr_act_type', type=str, default='gen', help='how to use user act during testing: generated or oracle')
    parser.add_argument('--sys_act_type', type=str, default='gen', help='how to use sys act during testing: generated, oracle')
    parser.add_argument('--goal_state_change', type=str, default='both', help='which goal state type to use: static, dynamic')

    # decoding
    parser.add_argument('--load_epoch', type=str, default='best', help='which checkpoint to load for evaluation')
    ...
    parser.add_argument('--max_dial_len', type=int, default=30, help='max dialogue length during interaction, usr-sys transition counts two')

    # user/agent interaction RL
    parser.add_argument('--update', type=str, default='joint', help='optimise user simulator and agent jointly or iteratively (only joint is implemented now)')
    ...
    parser.add_argument('--rl_dial_one_epoch', type=int, default=100, help='number of dialogues in one epoch')
    ...
    parser.add_argument('--reward_type', type=str, default='turn_reward',
                        help='which type of reward to use: turn_reward or dialogue_reward')
    ...
    # some experimental model design on dst, no need to change
    parser.add_argument('--dst_hst_len', type=int, default=100, help='take how many turns in dialogue history as input')
    parser.add_argument('--dst_pred_type', type=str, default='bs', help='predict turn-level understanding or whole belief state')
    parser.add_argument('--separate_value_list', type=str2bool, default=False, help='whether to use separate value list for each slot')
    parser.add_argument('--value_mask', type=str2bool, default=True, help='whether to use value mask on value logits')
    parser.add_argument('--remove_dontcare', type=str2bool, default=False, help='whether to remove dontcare in the value list')
    parser.add_argument('--attn_prev_bs', type=str2bool, default=True, help='whether to attend over belief state at previous turn.')

    # finetuning method
    parser.add_argument('--ft_method', type=str, default='naive')
    parser.add_argument('--fisher_sample', type=int, default=1000, help='number of dialogues from source domain used for calculating fisher matrix')
    parser.add_argument('--ewc_lambda', type=float, default=0.01)
    parser.add_argument('--src_train_path', type=str, default='', help='source domain training data')
    parser.add_argument('--src_valid_path', type=str, default='', help='source domain validation data')
    parser.add_argument('--src_test_path', type=str, default='', help='source domain testing data')
