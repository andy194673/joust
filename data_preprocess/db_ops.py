...
#				self.dbs[domain] = json.loads(f.read().lower())
		for domain in db_domains:
			self.dbs[domain] = json.load(open('data/raw_data/db/{}_db.json'.format(domain), 'r'))
...
##				match_result = [e['id'] for e in match_result]
				match_result = [e['trainID'] for e in match_result]
			else:
	...
	def querySQL(self, domain, constraints):
		if not self.sql_dbs:
			for dom in db_domains:
				db = 'data/raw_data/db/{}-dbase.db'.format(dom)
...
if __name__ == '__main__':
	dbPATHs = {
			'attraction': 'data/raw_data/db/attraction_db_processed.json',
			'hospital': 'data/raw_data/db/hospital_db_processed.json',
			'hotel': 'data/raw_data/db/hotel_db_processed.json',
			'police': 'data/raw_data/db/police_db_processed.json',
			'restaurant': 'data/raw_data/db/restaurant_db_processed.json',
			'taxi': 'data/raw_data/db/taxi_db_processed.json',
			'train': 'data/raw_data/db/train_db_processed.json',
		}
