...
for domain in domains:
	db = 'data/raw_data/db/{}-dbase.db'.format(domain)
...
def oneHotVector(num, domain, vector):
	...
	c = 3 # possible matched result, either 0, 1 or >2
	if num == 0:
		vector[idx * c: idx * c + c] = np.array([1, 0, 0])
	elif num == 1:
		vector[idx * c: idx * c + c] = np.array([0, 1, 0])
	elif num >= 2:
		vector[idx * c: idx * c + c] = np.array([0, 0, 1])
#	number_of_options = 6
#	if domain != 'train':
#		idx = domains.index(domain)
#		if num == 0:
